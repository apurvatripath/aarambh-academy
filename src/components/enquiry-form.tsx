    ...
    <section id="enquiry" className="scroll-mt-24 bg-navy-950 py-20 text-white sm:py-24">
      <div className="mx-auto grid max-w-7xl gap-12 px-5 sm:px-8 lg:grid-cols-[0.75fr_1.25fr] lg:px-10">
        <div>
          ...
          <div className="mt-8 rounded-xl border border-amber-400/40 bg-amber-400/10 p-5 text-sm leading-6 text-slate-100"><strong className="text-amber-400">Demonstration form only.</strong> Your details will not be stored, sent or used to contact you.</div>
        </div>
        <form onSubmit={submit} className="rounded-2xl bg-white p-6 text-navy-950 shadow-2xl sm:p-8">
          <div className="grid gap-6 sm:grid-cols-2">
            ...
            <label className="text-sm font-bold">Mobile number <span className="text-red-700">*</span>
              ...
              <span className="mt-2 block text-xs font-normal leading-5 text-slate-500">Used only to demonstrate the information a real enquiry form would collect.</span>
            </label>
            ...
            <label className="text-sm font-bold sm:col-span-2">Biggest academic concern <span className="text-red-700">*</span>
              <textarea name="concern" required rows={4} placeholder="For example: weak fundamentals, exam strategy, time management or choosing the right programme" className={fieldClass} />
            </label>
          </div>
          <label className="mt-6 flex items-start gap-3 text-sm leading-6 text-slate-700">
            ...
            <span>I agree that a coaching counsellor may contact me about this enquiry in a live implementation.</span>
          </label>
          <button type="submit" className="mt-7 min-h-12 w-full rounded-lg bg-amber-500 px-6 py-3 font-bold text-navy-950 hover:bg-amber-400">Request Counselling</button>
          {submitted && <p role="status" className="mt-5 rounded-lg bg-cream p-4 text-sm font-medium leading-6 text-navy-900">Demo enquiry prepared. In a live client website, this information would be sent securely for counselling follow-up. No data was transmitted here.</p>}
        </form>
      </div>
    </section>
  );
